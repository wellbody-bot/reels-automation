...
def build_briefing_markdown(results: dict, days: int, top_n: int = 10) -> str:
    ...
    for keyword, videos in results.items():
        ...
        if not videos:
            ...
            lines.append("")
            continue
        lines.append("| 순위 | 조회수 | 길이 | 제목 | 채널 | 링크 |")
        lines.append("|---|---|---|---|---|---|")
        for rank, v in enumerate(videos[:top_n], start=1):
            mins, secs = divmod(v["duration_sec"], 60)
            title = v["title"].replace("|", "\\|")
            lines.append(
                f"| {rank} | {v['view_count']:,} | {mins}:{secs:02d} | {title} | {v['channel']} | [보기]({v['url']}) |"
            )
        lines.append("")
    return "\n".join(lines)
...
def save_results(results: dict, output_dir: Path, days: int) -> Path:
    ...
    with csv_path.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["keyword", "title", "channel", "published_at", "view_count", "like_count", "comment_count", "duration_sec", "url"]
        )
        for keyword, videos in results.items():
            for v in videos:
                writer.writerow(
